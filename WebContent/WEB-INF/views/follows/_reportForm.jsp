...
<c:choose>
    ...
    <c:when test="${report.employee.id == login_employee.id}"></c:when>
    <c:when test="${followChecks[status.index] == 0}">
        <form method="POST" action="<c:url value='/follows/create' />">
            <input type="hidden" name="findItemNum" value="${findItemNum}">
            <input type="hidden" name="emp_id" value="${report.employee.id}">
            <input type="hidden" name="pageName" value="report">
